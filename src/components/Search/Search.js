...
export default function Search(props) {
    ...
    const searching = async () => {
        ...
        if (checkEmail.test(state)) {
            ...
            const dataFilter = await dataTable.filter(item => item.email.toLowerCase().includes(lowercaseQuery));
            setData1(dataFilter)
            setStatus("email");
        } else if (checkPhone.test(state)) {
            lowercaseQuery = parseInt(lowercaseQuery);
            const dataFilter = await dataTable.filter(item => parseInt(item.phone) === lowercaseQuery);
            setData1(dataFilter)
            console.log(dataFilter)
            setStatus("phone")
        } else {
            const dataFilter = await dataTable.filter(item => item.name.toLowerCase().includes(lowercaseQuery));
            setData1(dataFilter)
